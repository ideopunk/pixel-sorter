...
// RGB
export function redOrHueWithinThresholdCheck(pixel: PixelArray, min: number, max: number) {
	return pixel[0] >= min && pixel[0] < max;
}

export function greenOrSaturationWithinThresholdCheck(pixel: PixelArray, min: number, max: number) {
	return pixel[1] >= min && pixel[1] < max;
}
export function blueOrLightnessWithinThresholdCheck(pixel: PixelArray, min: number, max: number) {
	return pixel[2] >= min && pixel[2] < max;
}
export function alphaWithinThresholdCheck(pixel: PixelArray, min: number, max: number) {
	return pixel[3] >= min && pixel[3] < max;
}

// for use with rgb only
export function intensityWithinThresholdCheck(pixel: PixelArray, min: number, max: number) {
